...
r_init = load_pickle_(saved_arrays_dir+'AD300u_init_ki_xyz')
...
for k in range(n_states):
    for i in range(10):
        ...
        smiv.prepare_simulation_('AD_'+str(T)+'K', n_save+50,
                                 initial_conformer = r_init[k,i],
                                )
        print('ready to run k=',k+1,'/4, i=',i+1,'/10')
        print('n_save=',n_save)
...
save_pickle_(np.concatenate(xyz_ki,axis=0), trajectory_files_dir+'AD300u_sim_ki_xyz')
save_pickle_(np.concatenate(u_ki,axis=0), trajectory_files_dir+'AD300u_sim_ki_u')
save_pickle_(np.concatenate(Tu_ki,axis=0), trajectory_files_dir+'AD300u_sim_ki_Tu')

print('done AD'+str(T)+'u_multi')